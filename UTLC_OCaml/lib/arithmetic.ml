...
let church' (n : int) : C.term = (* C.Var "Todo: write the function described above" *)
   let rec aux x : C.term = 
      if x = 0 then (Var "x")
      else C.Ap (Var "f" , (aux (x-1)))
   in C.Lam ("f" , C.Lam ("x" , (aux n)))
...
(* 在这里像上面的 succ' 一样写 plus 的 lambda term。
   ...
   下同。 *)
let plus' = (* C.Var "Todo" *)
   clam "m" @@ clam "n" @@ clam "f" @@ clam "x" @@ ((Var "m" $$ Var "f") $$ (Var "n" $$ Var "f" $$ Var "x"))

(* try it *)
(* equiv (plus cThree cFour) cSeven *)
(* equiv ((to_locally_nameless plus')$(cThree)$(cFour)) cSeven;; *)


(* Define times (multiplication) *)
(* times n m = ? , or   times = λ n . λ m . ? *)
let times' = clam "m" @@ clam "n" @@ clam "f" @@ clam "x" @@ (Var "m" $$ (Var "n" $$ Var "f") $$ Var "x")

(* try it *)
(* equiv (times cThree cFour) cTwelf *)
(* equiv ((to_locally_nameless times')$(cThree)$(cFour)) cTwelf;; *)
...
(* and x y = ?
   and = λ x . λ y . ? *)
let bool_cond' = clam "p" @@ clam "a" @@ clam "b" @@ (Var "p" $$ Var "a" $$ Var "b")
let bool_and' = clam "x" @@ clam "y" @@ (bool_cond' $$ Var "y" $$ Var "x" $$ cFalse')
...
(* is_zero n = ?
   is_zero = λ n . ?` *)
let is_zero' = clam "f" @@ ((Var "f" $$ (clam "x" @@ cFalse')) $$ cTrue')
...
let kc' = clam "x" @@ clam "y" @@ Var "x"
let ic' = clam "x" @@ Var "x"
let gc' = clam "x" @@ clam "g" @@ clam "h" @@ (Var "h" $$ (Var "g" $$ Var "x"))
let pred' = clam "n" @@ clam "f" @@ clam "x" @@ (Var "n" $$ (gc' $$ Var "f") $$ (kc' $$ Var "x") $$ ic')
...
(* sum_u = ? *)
let sum_u' = clam "f" @@ clam "n" @@ (is_zero' $$ Var "n" $$ cZero' $$ (plus' $$ Var "n" $$ (Var "f" $$ (pred' $$ Var "n")) ) )
...
(* try it!
    sum  $ cFive |> nf
    equiv (sum  $ cFive |> nf) (church 15);;
    equiv (sum2  $ cFive |> nf) (church 30);;
    equiv (sum2  $ cFive |> nf) (calc $ cFive |> nf);;
    sum2 $ cFive |> nf
    calc $ cFive |> nf
*)