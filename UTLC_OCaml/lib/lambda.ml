...
module C = struct
  ...
  let st_ex3 = Lam ("y" , Lam ("z" , Ap (Lam ("a", Lam ("b", Var "a")), Ap (Var "z", Lam ("z" , Var "z" ))) ))
...
let to_locally_nameless (t : C.term) : term = (* Var (Free "_") *)
  let rec aux t d m : term =
    match t with
    | C.Var s -> let v = match Map.find m s with 
                         | None -> Var (Free s)
                         | Some i -> Var (Bound (d-i-1))
                 in v
    | C.Lam (s , t') -> Lam (aux t' (d+1) (Map.set m ~key:s ~data:d))
    | C.Ap (t1 , t2) -> Ap ((aux t1 d m), (aux t2 d m))
    in aux t 0 (Map.empty (module String))
...
let rec free_var (t : term) : string list = (* raise Todo *)
  let rec aux t acc : string list = 
    match t with
    | Var (Bound k) -> acc
    | Var (Free s) -> s::acc
    | Lam r -> aux r acc
    | Ap (e1 , e2) -> (aux e1 acc)@(aux e2 acc)
  in aux t []
...
let from_locally_nameless (t : term) : C.term = (* raise Todo *)
  let fg = Fresh.init ~base:"x" ~forbid:(free_var t) in
  let rec aux t d m g : C.term = 
    match t with
    | Var (Bound k) -> let v = match Map.find m (d-k-1) with 
                               | None -> if d < (k+1) 
                                         then raise (BadRepresentation "index too large") 
                                         else C.Var (match Fresh.get g with (n,ng) -> n)
                               | Some s -> C.Var s
                       in v
    | Var (Free s) -> C.Var s
    | Lam r -> let vname = (match Fresh.get g with (n,ng) -> n)
               in let ng = (match Fresh.get g with (n,ng) -> ng)
               in C.Lam (vname, aux r (d+1) (Map.set m ~key:d ~data:vname) ng)
    | Ap (e1 , e2) -> C.Ap ((aux e1 d m g), (aux e2 d m g))
  in aux t 0 (Map.empty (module Int)) fg
(* write a QuickCheck to check 
   t |> from_locally_nameless |> to_locally_nameless = t
   st_first |> from_locally_nameless |> to_locally_nameless
    *)
...
let rec subst_free (a : term) (x : string) (b : term) : term = (* raise Todo *)
  match a with
  | Var (Free s) -> if String.equal s x then b else a
  | Var (Bound _) -> a
...
let rec subst_bound (a : term) (b : term) : term = (* raise Todo *)
  let rec aux t d: term = 
    match t with
    | Var (Free _) -> t
    | Var (Bound k) -> if k = d then b else t
    | Lam r -> Lam (aux r (d+1))
    | Ap (e1 , e2) -> Ap ((aux e1 d), (aux e2 d))
  in aux a 0
...
module Good_nf = struct
  ...
  type mval = 
    | CLOS of ((string * mval) list * string * C.term)
    | NVar of string
    | NAp of (mval * mval)
    | MLazy of ((string * mval) list * C.term)


  let rec assv s env = match env with
  | [] -> raise Todo
  | (n, v)::tl -> if String.equal n s then v else assv s tl
  let extend env x v = (x, v)::env

  let rec valo env exp = match exp with
    | C.Lam (x, b) -> CLOS(env, x, b)
    | C.Var x -> assv x env
    | C.Ap (rator, rand) -> doap (valo env rator) (valo env rand)
  and doap f a = match f with
  | CLOS(env, x, b) -> MLazy ((extend env x a), b)
  | MLazy(env, b) -> let v = valo env b in doap v a
  | nf -> NAp (nf, a)

  let rec readback v = match v with
  | CLOS(env, x, b) -> C.Lam (x, readback (valo (extend env x (NVar x)) b))
  | MLazy(env, b) -> let v = valo env b in readback v
  | NVar x -> C.Var x
  | NAp (rator, rand) -> C.Ap (readback rator, readback rand)

  (* 你可能需要的 dterm 版的 subst_bound *)
  let dsubst_bound (a : dterm) (b : dterm) : dterm = (* raise Todo *)
    let rec aux t d: dterm = 
      match t with
      | DVar (DIndex i) -> if i = d then b else t
      | DLam r -> DLam (aux r (d+1))
      | DAp (e1, e2) -> DAp ((aux e1 d), (aux e2 d))
      | _ -> t
    in aux a 0
  ...
  let rec to_dterm (t : term) : dterm = (* raise Todo *)
    let rec aux t d: dterm = 
      ...
      | Lam r -> DLam (aux r (d+1))
      | Ap (Var (Bound k1) , Var (Bound k2)) -> DAp ((DVar (DLevel (d-k1-1))), (DVar (DLevel (d-k2-1))))
      | Ap (Var (Bound k) , e2) -> DAp ((DVar (DLevel (d-k-1))), (aux e2 d))
      | Ap (e1 , Var (Bound k)) -> DAp ((aux e1 d), (DVar (DLevel (d-k-1))))
      | Ap (e1 , e2) -> DAp ((aux e1 d), (aux e2 d))
    in aux t 0
  let rec from_dterm (t : dterm) : term = (* raise Todo *)
    let rec aux t d: term = 
      ...
      | DLam r -> Lam (aux r (d+1))
      | DAp (e1, e2) -> Ap ((aux e1 d), (aux e2 d))
    in aux t 0
  ...
  *)
  ...
  let nf (t : term) : term = (valo [] (from_locally_nameless t))|> readback |> to_locally_nameless
