...
(* second' = s k *)
(* 请你动手验证一下, 这两个确实是等价的 *)
(* second' = s k = λ y . λ z . (k z) (y z) = λ y . λ z . ((λ x . λ y . x) z) (y z)
           = λ y . λ z . (λ y . z) (y z) = λ y . λ z . z = λ x . λ y . y = second*)
...
let sk_ap = s $ (s $ k) |> from_locally_nameless

(* 下面来个难点的. 你可能需要**好好思考一下**如何做这个转换. *)
(* ex1  = λ a . a (λ c . a c) *)
(* ex1' = ? *)
let t_ex1 = clam "a" @@ (Var "a" $$ (clam "c" @@ (Var "a" $$ Var "c")))
let sk_ex1 = s $ (s $ k $ k) $ (s $ (s $ k)) |> from_locally_nameless
  ...
  *)
  ...
  let free x (e : ls) =
    ...
    let rec fv (e : ls) = match e with
      ...
      | Lam (x, e) -> List.filter (fun v -> v != x) (fv e)
  ...
  let rec translate (e : ls) : ls = match e with
    ...
    | Lam (x, Lam (y, e)) ->
      ...
      if free x e
      then translate (Lam (x, translate (Lam (y, e))))
      else failwith "error"
    (* clause 6. *)
    | Lam (x, App (e1, e2)) ->
      ...
      then App (App (Sl, (translate (Lam (x, e1)))), translate (Lam (x, e2)))
      else failwith "error"
    | Kl -> Kl
    | Sl -> Sl
    | Il -> Il
    ...
    let convert (e : lambda) : ski =
        ...
        ski_of_ls ls_comb
  let rec c_to_ls (t: C.term) : lambda = 
    match t with
    | C.Var s -> Var s
    | C.Lam (s , t') -> Lam (s , c_to_ls t')
    | C.Ap (t1 , t2) -> App (c_to_ls t1 , c_to_ls t2)
  let rec ski_to_sk (t: ski): expr = 
    match t with
    | I -> App (S , App (S, K))
    | K -> K
    | S -> S
    | T(e1, e2) -> App (ski_to_sk e1 , ski_to_sk e2)
  let from_lambda (t : term) : expr =
    t |> nf |> from_locally_nameless |> c_to_ls |> convert |> ski_to_sk

  (* i |> Systematic_SK.from_lambda |> Systematic_SK.to_lambda |> nf;; *)
